...
-(id) init {
    if (self = [super init]) {
        ...
        //init the 
        previewData = [[NSMutableData alloc]init];
...
-(bool) imageLengthCheck:(int) length{
    ...
    Debug(@"remaining is %d  and length is %d",remaining,length);
...
- (void) readImageInfo 
{
	...
	if (len < 0 ){
	   // throw new JpegException("Erroneous JPEG marker length");
        Debug(@"ERROR: Length is negative in reading image info ");
        return;
    }
    
    if (len > imageLength){
        Debug(@"ERROR: Length is bigger than image length ");
        return;
    }
    
    Debug(@"Length in image info %i ",len);
...
-(void) scanImageData: (NSData*) jpegData {
    ...
    Debug(@"Length of image %d", imageLength);
               ...
               // those out too when in -verbose mode.
                {
                    ...
                    NSData* commentData;
                    if (val==M_APP0 || val== M_APP1) {
                        commentData = [self processComment];
                        NSNumber* key = [[NSNumber alloc]initWithInt:val];
                        //kc test 20151026, we don't need to set comment.
...
//                        [self.keyedHeaders  setObject:commentData forKey:key];
               ...
               default:	          // Anything else just gets skipped
            {
                ...
                errorExit = [self skipVariable];  // we assume it has a parameter count...
            }
               break;
               }     
...
#if 1
                //endOfEXFPtr means end of the start of preview.
                NSLog(@"kc test:%0x %0x %0x %0x",endOfEXFPtr[0], endOfEXFPtr[1], endOfEXFPtr[2], endOfEXFPtr[3]);
                
//                NSInteger previewTag = [self readNext2bytes];
                while(([self readNext2bytes])==0xFFE2)
                {//previewTag
                    previewLength = [self readNext2bytes];//read 2 bytes--It is
                    NSLog(@"kc test:%0x %0x %0x %0x",imageBytePtr[0], imageBytePtr[1], imageBytePtr[2], imageBytePtr[3]);
                    NSLog(@"kc test:previewLength is %0x", previewLength);
                    [previewData appendBytes:imageBytePtr length:previewLength-2];
                    //move the pointer.
                    imageBytePtr=imageBytePtr+(previewLength-2);
                    NSLog(@"kc test:%0x %0x %0x %0x",imageBytePtr[0], imageBytePtr[1], imageBytePtr[2], imageBytePtr[3]);
                }
                if(previewData.length)
                    self.previewImage = [UIImage imageWithData:previewData];
                
                //try to free the remain data.
                endOfEXFPtr = imageBytePtr;
#endif
                CFIndex offset = endOfEXFPtr - imageStartPtr;
                if(offset>0 && offset<imageLength)
                {
                    NSData* theRemainingdata = [[NSData alloc] initWithBytes:endOfEXFPtr length:imageLength - offset];
            ...
            {
                NSLog(@"ERROR: endOfEXFPtr is NULL");
            }
        }
        endOfEXFPtr = NULL;
        imageStartPtr = NULL;
